...
def generate_client(client_id):
    """Generate a client file for the given client ID"""
    client_dir = f"client{client_id}"
    os.makedirs(client_dir, exist_ok=True)
    
    client_code = f'''import flwr as fl
import numpy as np
import pandas as pd
...
def main():
    """Start the client"""
    client_id = {client_id}  # This client's ID
...
'''
    ...
    with open(f"{client_dir}/client.py", "w") as f:
        f.write(client_code)
    
    print(f"Generated client {client_id}")

def main():
    """Generate all remaining clients"""
    print("Generating remaining client files...")
    
    # Generate clients 6-10
    for client_id in range(6, 11):
        generate_client(client_id)
    
    print("All client files generated!")
