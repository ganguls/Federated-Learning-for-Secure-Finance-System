#!/usr/bin/env python3
"""
Final working version with complete Flask bypass for attack detection
"""
...
# COMPLETE BYPASS: Use a separate Python script for attack detection
@app.route('/api/detection/run_demo', methods=['POST'])
def run_detection_demo():
    """Run attack detection demo - COMPLETE FLASK BYPASS"""
    try:
        ...
        # Call external Python script to bypass Flask completely
        import subprocess
        import tempfile
        import os
...
result = {{
    ...
    'client_details': [
        {{
            'client_id': i,
            'loss': float(loss),
            'is_malicious': i in true_malicious,
...
'''
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return Response(
                    json.dumps({'success': False, 'error': f'Script failed: {result.stderr}'}),
                    status=500,
                    mimetype='application/json'
                )
        finally:
