...
def check_data_files():
    ...
    # Check if we have at least 5 client data files
    client_files = list(data_path.glob("client_*.csv"))
    if len(client_files) < 5:
        print(f"Only {len(client_files)} client data files found. Need at least 5.")
        return run_data_preprocessing()
...
def start_clients(num_clients=5):
...
def main():
    ...
    # Start clients
    client_processes = start_clients(num_clients=5)
